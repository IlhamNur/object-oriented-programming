...
class time { // Define class called time
	...
	time(int h, int m, int s) { // 2nd constructor initialize data to fixed values
		hours = h;
		minutes = m;
		seconds = s;
	}
...
void display(time A) { // Friend function display which takes an object of class time type as argument	
    ...
    cout << setw(2) << setfill('0') << A.hours << ":" << setw(2) << setfill('0') << A.minutes << ":" << setw(2) << setfill('0') << A.seconds << endl;
}
